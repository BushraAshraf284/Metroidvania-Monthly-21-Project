...
public class Upgrade : MonoBehaviour
{
	...
	public enum upgrade{ShockProng, ShockSpike, Missile, HomingMissiles, JetBooster, VerticalBooster, Sword, BatteryPiece, HeartPiece, ShieldPiece, RepairKit,ICChip};
    ...
    void OnTriggerEnter(Collider other)
    {
        if(other.gameObject.tag == "Player"){
            ...
            else if (whatUpgrade == upgrade.ICChip)
            {
                if (other.transform.root.gameObject.GetComponent<UpgradeTracker>() != null) 
                {
                    other.transform.root.gameObject.GetComponent<UpgradeTracker>().GetICChip();
                    AudioManager.instance.PlayOneShot(FMODEvents.instance.upgradeCollectedSound, this.transform.position);
                }
            }
	        isPickedUp = true;
