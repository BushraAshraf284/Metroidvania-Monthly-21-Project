Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/99ffa134a52d7af459e3ae9d79af5338"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float totalPixels = float4(1280,720,0,0).x * float4(1280,720,0,0).y;
				float2 uvPixel = float2((floor((1 - uv.y) * float4(1280,720,0,0).y)) / float4(1280,720,0,0).y, (floor(uv.x * float4(1280,720,0,0).x)) / float4(1280,720,0,0).x);
				float currentPixelNumber = float4(1280,720,0,0).x * (float4(1280,720,0,0).y * uvPixel.x) + float4(1280,720,0,0).y * uvPixel.y;
			...
			{
				videoTexture.rgb = applyBacklight(videoTexture, (1.1 /*_VideoBacklight*/) * .01);
				applyVideoSettings(videoTexture.rgb);
				videoTexture.rgb = videoTexture * pixels * (1.1 /*_VideoBacklight*/);
			}
			...
			void calculateCRT(inout float4 videoTexture, float3 pixels, float2 uv)
			{
				...
				applyVideoSettings(videoTexture.rgb);
				videoTexture.rgb = videoTexture * pixels * brightness * (1.1 /*_VideoBacklight*/);
			}
			...
			{
				applyVideoSettings(videoTexture.rgb);
				float3 projectorColor = videoTexture * (1.1 /*_VideoBacklight*/);
			...
			void applyVideoEffectsMainTex(inout float4 mainTexture, in PoiMesh poiMesh)
			{
				...
				if((0.0 /*_VideoPixelateToResolution*/))
				{
					...
					uvs = sharpSample(float4(1/float4(1280,720,0,0).xy, float4(1280,720,0,0).xy), uvs);
					uvs = poiUV(uvs, float4(1,1,0.48,-1.17)) + _Time.x * float4(0,0,0,0);
			...
			{
				#if defined(PROP_VIDEOPIXELTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float3 pixels = tex2D(_VideoPixelTexture, poiUV(poiMesh.uv[(0.0 /*_VideoPixelTextureUV*/)], float4(1,1,0,0)) * float4(1280,720,0,0));
				#else
				float3 pixels = 1;
				#endif
				float2 uvs = poiMesh.uv[(0.0 /*_MainTexUV*/)];
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex, _MainTex, poiUV(mainUV, float4(1,1,0.48,-1.17)), float4(0,0,0,0), (0.0 /*_MainTexStochastic*/));
