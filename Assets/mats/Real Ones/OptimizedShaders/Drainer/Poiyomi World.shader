Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/35f73e4b13c9b2447b97b64945cbdee7"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = (4.6 /*_EmissionStrength*/);
				...
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_EmissionMapUV*/)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0,0.06507418,0.659754,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * (1.0 /*_EmissionCenterOutSpeed*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0,0.06507418,0.659754,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0,0.06507418,0.659754,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				#endif
				...
				emissionColor0 = hueShift(emissionColor0, frac((0.159 /*_EmissionHueShift*/) + (0.0 /*_EmissionHueShiftSpeed*/) * _Time.x) * (0.0 /*_EmissionHueShiftEnabled*/));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
			...
			float3 RandomColorFromPoint(float2 rando)
			{
				...
				fixed saturation = lerp(float4(0.3309446,0.5309446,0,1).x, float4(0.3309446,0.5309446,0,1).y, rando.x);
				fixed value = lerp(float4(0.6175896,0.8175895,0,1).x, float4(0.6175896,0.8175895,0,1).y, rando.y);
			...
			{
				float2 st = frac(poiMesh.uv[(8.0 /*_GlitterUV*/)] + float4(0,0,0,0).xy * _Time.x) * (306.6 /*_GlitterFrequency*/);
				...
				for (int j = -1; j <= 1; j++)
				{
					for (int i = -1; i <= 1; i++)
					{
						...
						float2 rando = pos;
						pos = 0.5 + 0.5 * sin((0.0 /*_GlitterJitter*/) * 6.2831 * pos);
				...
				float size = (0.535 /*_GlitterSize*/);
				
				if ((1.0 /*_GlitterRandomSize*/))
				{
					size = remapClamped(0, 1, randomFromPoint, float4(0.6,1,0,1).x, float4(0.6,1,0,1).y);
				}
				half glitterAlpha = 1;
				switch((1.0 /*_GlitterShape*/))
				{
					...
					if ((1.0 /*_GlitterRandomRotation*/) == 1 || (-242.5 /*_GlitterTextureRotation*/) != 0)
					{
						float2 center = float2(0, 0);
						float randomBoy = 0;
						
						if ((1.0 /*_GlitterRandomRotation*/))
						{
							randomBoy = random(randoPoint);
						}
						float theta = radians((randomBoy + _Time.x * (-242.5 /*_GlitterTextureRotation*/)) * 360);
						float cs = cos(theta);
						float sn = sin(theta);
				...
				half3 glitterColor = poiThemeColor(poiMods, float4(0,0.1468581,0.9056604,1), (0.0 /*_GlitterColorThemeIndex*/));
				float3 norm = lerp(poiMesh.normals[0], poiMesh.normals[1], (1.0 /*_GlitterUseNormals*/));
				float3 randomRotation = 0;
				switch((1.0 /*_GlitterMode*/))
				{
					case 0:
					...
					finalGlitter = lerp(0, (0.293 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.424 /*_GlitterBias*/)), poiCam.viewDir)), (682.0 /*_GlitterContrast*/)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * (10.0 /*_GlitterSpeed*/)) * (100.0 /*_glitterFrequencyLinearEmissive*/) - ((100.0 /*_glitterFrequencyLinearEmissive*/) - 1);
					finalGlitter = max((0.293 /*_GlitterMinBrightness*/) * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * (0.034 /*_GlitterCenterSize*/) * 10));
					break;
					case 2:
					if ((10.0 /*_GlitterSpeed*/) > 0)
					{
					...
					finalGlitter = lerp(0, (0.293 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (682.0 /*_GlitterContrast*/)), 0);
					glitterColor *= poiLight.directColor;
					finalGlitter *= glitterAlpha;
					break;
				}
				glitterColor *= lerp(1, poiFragData.baseColor, (0.877 /*_GlitterUseSurfaceColor*/));
				#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				...
				if ((1.0 /*_GlitterRandomRotation*/) == 1 || (-242.5 /*_GlitterTextureRotation*/) != 0 && !(1.0 /*_GlitterShape*/))
				{
				...
				float glitterMask = 1;
				#endif
				glitterMask *= lerp(1, poiLight.rampedLightMap, (0.031 /*_GlitterHideInShadow*/));
